...
bool Player::canWalkthrough(const Creature *creature) const
{
	...
	if ((((g_game.getWorldType() == WORLDTYPE_OPTIONAL && !player->isEnemy(this, true) &&
		  ...
		  player->getTile()->hasFlag(TILESTATE_PROTECTIONZONE) || (player->getVocation()->isAttackable() && player->getLevel() < (uint32_t)g_config.getNumber(ConfigManager::PROTECTION_LEVEL))) &&
		 player->getTile()->ground &&
		 Item::items[player->getTile()->ground->getID()].walkStack) &&
...
void Player::onAttackedCreatureChangeZone(ZoneType_t zone)
{
	...
	else if (zone == ZONE_OPEN && g_game.getWorldType() == WORLDTYPE_OPTIONAL && attackedCreature->getPlayer() && !attackedCreature->getPlayer()->isEnemy(this, true))
	{
...
void Player::onAttackedCreature(Creature *target)
{
	...
	if (Combat::isInPvpZone(this, targetPlayer) || isPartner(targetPlayer) ||
		isAlly(targetPlayer) ||
		(g_config.getBool(ConfigManager::ALLOW_FIGHTBACK) && targetPlayer->hasAttacked(this) && !targetPlayer->isEnemy(this, false)))
		return;
...
bool Player::isEnemy(const Player *player, bool allies) const
{
	...
	return !warMap.empty() && (((g_game.getWorldType() != WORLDTYPE_OPTIONAL || g_config.getBool(
																					ConfigManager::OPTIONAL_WAR_ATTACK_ALLY)) &&
								allies && guildId == guild) ||
